...
#pragma omp parallel for
    for(int a = 0; a < nr_of_dust_species; a++)
    {
        ...
        for(int w = 0; w < nr_of_wavelength; w++)
        {
            ...
            uint percentage = uint(100.0 * float(per_counter) / float(max_counter));
...
#pragma omp critical
            ...
            sca_mat[a][w] = new double***[nr_ofIncidentAngles];
            for(int inc = 0; inc < nr_ofIncidentAngles; inc++)
            {
                // Fourth init of the scattering matrix
                sca_mat[a][w][inc] = new double**[nr_ofScatPhi];
                for(int sph = 0; sph < nr_ofScatPhi; sph++)
                {
                    // Fifth init of the scattering matrix
                    sca_mat[a][w][inc][sph] = new double*[nr_ofScatTheta];
                    for(int sth = 0; sth < nr_ofScatTheta; sth++)
                    {
                        // sixth init of the scattering matrix
                        sca_mat[a][w][inc][sph][sth] = new double[nr_ofScatMatElements];

                        // Set each element to zero
                        for(int mat = 0; mat < nr_ofScatMatElements; mat++)
...
bool CDustComponent::readDustParameterFile(parameter & param, uint dust_component_choice)
{
    ...
    while(getline(reader, line))
    {
        ...
        switch(cmd_counter)
        {
            ...
            case 2:
                ...
                nr_ofIncidentAngles = (uint) values[2];
                ...
                Qtrq_wl = new spline[nr_of_dust_species * nr_ofIncidentAngles];
                HG_g_factor_wl = new spline[nr_of_dust_species * nr_ofIncidentAngles];
            ...
            default:
                ...
                // Each line contains NR_OF_EFF - 1 plus 2 times nr_ofIncidentAngles values
                if(values.size() == NR_OF_EFF - 1 + 2 * nr_ofIncidentAngles)
                {
                    ...
                    for(uint i_inc = 0; i_inc < nr_ofIncidentAngles; i_inc++)
                    {
                        // At the first wavelength, resize the splines for the wavelengths
                        if(w == 0)
                        {
                            Qtrq_wl[a * nr_ofIncidentAngles + i_inc].resize(nr_of_wavelength_dustcat);
                            HG_g_factor_wl[a * nr_ofIncidentAngles + i_inc].resize(nr_of_wavelength_dustcat);
                        }

                        // Set the radiative torque efficiency
                        Qtrq_wl[a * nr_ofIncidentAngles + i_inc].setValue(w, wavelength_list_dustcat[w],
                            values[NR_OF_EFF - 1 + i_inc]);

                        // Set the Henyey-Greenstein factor
                        HG_g_factor_wl[a * nr_ofIncidentAngles + i_inc].setValue(w, wavelength_list_dustcat[w],
                            values[NR_OF_EFF - 1 + i_inc + nr_ofIncidentAngles]);
                    }
                ...
                if(w == nr_of_wavelength_dustcat - 1)
                {
                    ...
                    for(uint i_inc = 0; i_inc < nr_ofIncidentAngles; i_inc++)
                    {
                        Qtrq_wl[a * nr_ofIncidentAngles + i_inc].createSpline();
                        HG_g_factor_wl[a * nr_ofIncidentAngles + i_inc].createSpline();
                    }
                }
    ...
    // If not a line per combination of grain size and wavelength was found in the catalog, show error
    ...
    for(uint a = 0; a < nr_of_dust_species; a++)
    {
        ...
        for(uint w = 0; w < nr_of_wavelength; w++)
        {
            ...
            Qtrq[w * nr_of_dust_species + a].resize(nr_ofIncidentAngles);
            HG_g_factor[w * nr_of_dust_species + a].resize(nr_ofIncidentAngles);

            if(sizeIndexUsed(a, a_min_global, a_max_global))
            {
                // Calculate the difference between two incident angles
                double d_ang;
                if(nr_ofIncidentAngles > 1)
                    d_ang = PI / double(nr_ofIncidentAngles - 1);
                else
                    d_ang = 1;
                ...
                for(uint i_inc = 0; i_inc < nr_ofIncidentAngles; i_inc++)
                {
                    Qtrq[w * nr_of_dust_species + a].setValue(i_inc, i_inc * d_ang,
                        Qtrq_wl[a * nr_ofIncidentAngles + i_inc].getValue(wavelength_list[w]));
                    HG_g_factor[w * nr_of_dust_species + a].setValue(i_inc, i_inc * d_ang,
                        HG_g_factor_wl[a * nr_ofIncidentAngles + i_inc].getValue(wavelength_list[w], EXPOL_CONST));
                }
                ...
                {
                    double tmpQext = 1.0 / 3.0 * (2.0 * eff_wl[a * NR_OF_EFF + 0].getValue(wavelength_list[w]) +
                        eff_wl[a * NR_OF_EFF + 1].getValue(wavelength_list[w]));
                    double tmpQabs = 1.0 / 3.0 * (2.0 * eff_wl[a * NR_OF_EFF + 2].getValue(wavelength_list[w]) + 
                        eff_wl[a * NR_OF_EFF + 3].getValue(wavelength_list[w]));
                    double tmpQsca = 1.0 / 3.0 * (2.0 * eff_wl[a * NR_OF_EFF + 4].getValue(wavelength_list[w]) + 
...
bool CDustComponent::readScatteringMatrices(string path, uint nr_of_wavelength_dustcat, dlist wavelength_list_dustcat)
{
    // Init variables
    ...
    while(getline(inf_reader, line))
    {
        ...
        switch(cmd_counter)
        {
            case 1:
                ...
                // The number of incident angles
                if(values[2] != nr_ofIncidentAngles)
                {
                    cout << "ERROR: " << inf_filename.c_str() << " line " << line_counter << "!" << endl;
                ...
                nr_ofScatPhi = uint(values[3]);

                // The number of theta angles (outgoing radiation)
                nr_ofScatTheta = uint(values[4]);
                break;
            ...
            case 2:
                ...
                nr_ofScatMatElements = uint(values[0]);
                break;
            ...
            case 3:
                ...
                // The relation which scattering matrix entry is used at which position in the 4x4 matrix
                for(uint i = 0; i < 16; i++)
                {
    ...
    // If scattering matrix is empty, disable mie scattering for the corresponding dust component
    ...
    // Init maximum counter value
    uint max_counter = nr_ofIncidentAngles * nr_of_dust_species * nr_ofScatPhi;

    // First init of the scattering matrix linear
    linear ***** sca_mat_wl = new linear****[nr_of_dust_species];

#pragma omp parallel for
    for(int a = 0; a < nr_of_dust_species; a++)
    {
        // Second init of the scattering matrix linear
        sca_mat_wl[a] = new linear***[nr_ofIncidentAngles];
        for(int inc = 0; inc < nr_ofIncidentAngles; inc++)
        {
            // Third init of the scattering matrix linear
            sca_mat_wl[a][inc] = new linear**[nr_ofScatPhi];
            for(int sph = 0; sph < nr_ofScatPhi; sph++)
            {
                // Increase counter used to show progress
                per_counter++;
...
#pragma omp critical
                ...
                // Fourth init of the scattering matrix linear
                sca_mat_wl[a][inc][sph] = new linear*[nr_ofScatTheta];
                for(int sth = 0; sth < nr_ofScatTheta; sth++)
                {
                    // Fifth init of the scattering matrix linear
                    sca_mat_wl[a][inc][sph][sth] = new linear[nr_ofScatMatElements];
...
#pragma omp critical
            ...
            for(uint inc = 0; inc < nr_ofIncidentAngles; inc++)
            {
                for(uint sph = 0; sph < nr_ofScatPhi; sph++)
                {
                    ...
                    {
                        for(uint mat = 0; mat < nr_ofScatMatElements; mat++)
                        {
                            // Read value from binary file
                            bin_reader.read((char*) &tmp_val, 4);

                            // Set value of linear to the read value
                            sca_mat_wl[a][inc][sph][sth][mat].setValue(w, wavelength_list_dustcat[w], tmp_val);
                        }
                    }
                }
            }
                ...
                for(uint a = 0; a < nr_of_dust_species; a++)
                {
                    ...
                    if(calorimetry_type == CALO_HEAT_CAP)
                    {
                        if(t == 0)
                            ...
                            enthalpy[a][t] = enthalpy[a][t - 1] + tmp_value *
                                (calorimetry_temperatures[t] - calorimetry_temperatures[t - 1]);
                    }
...
{
    // Init arrays of splines/linear
    avg_scattering_frac = new spline*[nr_of_dust_species];
    phase_pdf = new linear*[nr_of_dust_species];

    // Calculate differences between two angles
    double d_ang = PI / double(nr_ofScatTheta - 1);
...
#pragma omp parallel for
    ...
    {
        // Init pointer arrays
        double * S11_tmp = new double[nr_ofScatTheta];
        double * S11_solid_angle = new double[nr_ofScatTheta];
        double * tmp_scat_frac = new double[nr_ofScatTheta];

        // Init arrays of linear
        avg_scattering_frac[a] = new spline[nr_of_wavelength];
        phase_pdf[a] = new linear[nr_of_wavelength];
        for(uint w = 0; w < nr_of_wavelength; w++)
        {
            // Resize splines for scattering angles
            avg_scattering_frac[a][w].resize(nr_ofScatTheta);
            phase_pdf[a][w].resize(nr_ofScatTheta);

            for(uint th = 0; th < nr_ofScatTheta; th++)
            {
                // Calculate the scattering propability in a certain direction
                S11_tmp[th] = double(sca_mat[a][w][0][0][th][0]);

                // Calculate the modified angle for integration
                S11_solid_angle[th] = PIx2 * cos(double(th) * d_ang);

                // Set the current value for the integration array tmp_scat_frac
                if(th == 0)
                    tmp_scat_frac[th] = 0;
                else
                    tmp_scat_frac[th] = -CMathFunctions::integ(S11_solid_angle, S11_tmp, 0, th);
            }

            // Integral of the scattering S11 value over the full sphere
            double int_scat_frac = tmp_scat_frac[nr_ofScatTheta - 1];

            for(uint th = 0; th < nr_ofScatTheta; th++)
            {
                if(int_scat_frac > 0)
                {
                    // Set the cumulative distribution function of being scattered at a certain angle
                    avg_scattering_frac[a][w].setValue(th, tmp_scat_frac[th] / int_scat_frac, double(th) * d_ang);

                    // Set the phase function of how much is scattered at a certain angle
                    phase_pdf[a][w].setValue(th, double(th) * d_ang, S11_tmp[th] / int_scat_frac);
                }
            }
...
#pragma omp parallel for
    ...
    for(uint t = 0; t < nr_of_temperatures; t++)
    {
        // Init array of mean absorption cross-section times dPlanck/dT for each wavelength and grain size
        double ** pl_mean;
        ...
        for(uint a = 0; a < nr_of_dust_species; a++)
            CMathFunctions::probListInteg(wavelength_list, pl_mean[a], avg_planck_frac[t * nr_of_dust_species + a]);
...
bool CDustComponent::calcSizeDistribution(dlist values, double * mass)
{
        ...
        if(size_keyword.find("-ed") != std::string::npos)
        {
            for(int a = 0; a < nr_of_dust_species; a++)
                if(a_eff[a] > size_parameter[1])
                    a_eff_3_5[a] *= exp(-pow((a_eff[a] - size_parameter[1]) / size_parameter[2],
                        size_parameter[3]));
        }
    ...
    else if(size_keyword.find("zda") != std::string::npos)
    {
        ...
        for(int a = 0; a < nr_of_dust_species; a++)
        {
            ...
            if(size_parameter[2] != 0 || size_parameter[3] != 0 || size_parameter[4] != 0)
                log_g -= size_parameter[3] * pow(abs(log10(a_eff_micron / size_parameter[4])), size_parameter[2]);
            if(size_parameter[5] != 0 || size_parameter[6] != 0 || size_parameter[7] != 0)
                log_g -= size_parameter[6] * pow(abs(log10(a_eff_micron / size_parameter[7])), size_parameter[5]);
    ...
    for(int a = 0; a < nr_of_dust_species; a++)
    {
        ...
        // For PAHs, their mass can be calculated as standard grains or specifically for PAHs
...
bool CDustComponent::add(double frac, CDustComponent * comp)
{
    ...
    if(comp->getComponentId() == 0)
    {
        ...
        nr_ofIncidentAngles = comp->getNrOfIncidentAngles();
        nr_ofScatTheta = comp->getNrOfScatTheta();
        nr_ofScatPhi = comp->getNrOfScatPhi();
        nr_ofScatMatElements = comp->getNrOfScatMatElements();
        ...
        {
            Qtrq[i].resize(nr_ofIncidentAngles);
            HG_g_factor[i].resize(nr_ofIncidentAngles);
        }
...
void CDustComponent::calcCrossSections(CGridBasic * grid, photon_package * pp, uint i_density, uint a,
        cross_sections & cs)
{
...
void CDustComponent::convertTempInQB(CGridBasic * grid, cell_basic * cell, uint i_density,
        double min_gas_density, bool use_gas_temp)
{
    ...
    // Get temperature from grid as offset (assuming only ONE dust/gas temperature in input grid)
    if(use_gas_temp)
...
double CDustComponent::updateDustTemperature(CGridBasic * grid, photon_package * pp,
...
void CDustComponent::calcTemperature(CGridBasic * grid, cell_basic * cell,
...
double CDustComponent::calcGoldReductionFactor(Vector3D & v, Vector3D & B)
{
    ...
    if(s < 0)
    {
        if(g < 0)
        {
            cossq_beta = (sqrt(-g) * asin(sqrt(-s / (1 + g))))
                    / (s * atan(sqrt((s * g) / (1 + s + g)))) - 1 / s;
        }
        else
        {
            cossq_beta = (sqrt(g) * asin(sqrt(-s / (1 + g))))
    ...
    else
    {
        if(g < 0)
        {
            cossq_beta = (sqrt(-g) * asinh(sqrt(s / (1 + g))))
                    / (s * atanh(sqrt(-(s * g) / (1 + s + g)))) - 1 / s;
        }
        else
        {
            cossq_beta = (sqrt(g) * asinh(sqrt(s / (1 + g))))
        ...
        dlist & wavelength_list_full)
{
    ...
    for(uint a = 0; a < nr_of_dust_species; a++)
    {
        ...
        if(a_eff[a] <= getStochasticHeatingMaxSize() && sizeIndexUsed(a, a_min, a_max))
        {
            ...
            for(uint w = 0; w < WL_STEPS; w++)
                abs_rate_per_wl.setValue(w, wavelength_list_full[w],
...
double CDustComponent::getCalorimetryA(uint a, uint f, uint i, spline & abs_rate_per_wl)
{
    ...
    if(f > i)
    {
        ...
        if(enthalpy_diff > 0)
        {
            ...
            // Calculate A
            res = abs_rate_per_wl.getValue(wavelength) * con_h * con_c *
                    getEnthalpyBinWidth(a, f) / pow(enthalpy_diff, 3);
        }
    }
    else if(f == i - 1)
    {
...
long double * CDustComponent::getStochasticProbability(uint a, spline & abs_rate_per_wl)
{
    ...
    // Go through each possible transition between temperature states and set B matrix values
...
double CDustComponent::calcEmissivities(CGridBasic * grid, photon_package * pp, uint i_density)
{
        ...
        if(sizeIndexUsed(a, a_min, a_max))
        {
            ...
            if(a_eff[a] <= getStochasticHeatingMaxSize())
            {
                ...
                for(uint t = 0; t < getNrOfCalorimetryTemperatures(); t++)
                {
                    ...
                    // Get propability that the dust grains have the current calorimetric temperature
                    double temp_probability = grid->getDustTempProbability(pp, i_density, a, t);

                    // Add relative emissivity from this temperature
                    pl_abs_tmp[a] += getCabsMean(a, w) * rel_weight * temp_probability * getTabPlanck(w, temp_dust);
                }
            }
            else
            {
    ...
    // Perform integration to obtain a more precise result for the efficient emission and cross section
...
StokesVector CDustComponent::calcEmissivitiesEmi(CGridBasic * grid, photon_package * pp, uint i_density,
        double phi, double energy, Vector3D en_dir)
{
    ...
    for(uint a = 0; a < nr_of_dust_species; a++)
    {
        if(sizeIndexUsed(a, a_min, a_max))
        {
            ...
            // Get cross sections and relative weight of the current dust grain size
            calcCrossSections(grid, pp, i_density, a, cs);
            ...
            if(energy > 1e-200)
            {
                ...
                // Multiply energy with scattered fraction in the theta angle and the scattering cross-section
...
void CDustComponent::calcExtCrossSections(CGridBasic * grid, photon_package * pp, uint i_density,
        ...
        Vector3D obs_ex, Vector3D dir_obs, double albedo)
{
    switch(phID)
    {
        ...
        default:
        {
            ...
            // Determination of the scattering angle (phi, theta) towards the observing map in the photon frame.
            // Get the rotation matrix of the photon (photon space to lab space)
        ...
        Vector3D obs_ex, Vector3D dir_obs, double albedo)
{
    ...
    double rot_angle_phot_obs = CMathFunctions::getRotationAngleObserver(obs_ex, pp_res.getEX(), pp_res.getEY());
...
double CDustComponent::getCellEmission(CGridBasic * grid, photon_package * pp, uint i_density)
{
    ...
    double total_energy = 1 / weight * dens * vol * PIx4 * CMathFunctions::integ_dust_size(a_eff, energy,
        nr_of_dust_species, a_min, a_max);
    ...
    if(param.getCommand() == CMD_DUST_EMISSION)
    {
        ...
        for(uint i = 0; i < dust_ray_detectors.size(); i += NR_OF_RAY_DET)
        {
            ...
            if(uint(dust_ray_detectors[i + 2]) > 1)
                ...
                cout << "- Emission detetector " << (pos + 1) << "   : "
                    << dust_ray_detectors[i + 0] << " [m]" << endl;
        }
    }
    ...
    if(param.getCommand() == CMD_DUST_SCATTERING)
    {
        ...
        for(uint i = 0; i < dust_mc_detectors.size(); i += NR_OF_MC_DET)
        {
            ...
            if(uint(dust_mc_detectors[i + 2]) > 1)
                ...
                cout << "- Scattering detetector " << (pos + 1) << "   : "
                    << dust_mc_detectors[i + 0] << " [m]" << endl;
        }
    }
...
bool CDustMixture::mixComponents(parameter & param, uint i_mixture)
{
    ...
    if(nr_of_components == 1)
    {
        ...
        preCalcDustProperties(param, i_mixture);
    ...
    uint nr_ofIncidentAngles = single_component[0].getNrOfIncidentAngles();
    ...
    for(uint i_comp = 0; i_comp < nr_of_components; i_comp++)
    {
        ...
        if(nr_ofIncidentAngles != single_component[i_comp].getNrOfIncidentAngles())
        {
            cout << "ERROR: Component Nr. " << i_comp + 1 << " has a different amount of incident angles!" << endl;
            return false;
        }

        // If scattering matrix was not loaded for one component -> not loaded for the mixture
        if(!single_component[i_comp].getScatLoaded())
            mixed_component[i_mixture].setScatLoaded(false);

        // If calorimetry data was not loaded for one component .> not loaded for the mixture
...
bool CDustMixture::preCalcDustProperties(parameter & param, uint i_mixture)
{
    ...
    if(mixed_component[i_mixture].getCalorimetryLoaded() && param.getStochasticHeatingMaxSize() > mixed_component[i_mixture].getSizeMin())
    {
    ...
    // If scattering matrix was loaded, pre-calculate phase function and scattering distribution
