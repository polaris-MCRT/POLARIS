...
bool CGridVoronoi::loadGridFromBinrayFile(parameter & param, uint _data_len)
{
    ...
    if(bin_reader.fail())
    {
        cout << "ERROR: Can not load binary voronoi grid file: \n";
        cout << filename << "\n\n";
        return false;
    }
    ...
    if(max_cells < 4)
    {
        cout << "ERROR: A minimum of at least four voronoi cells is required!     \n";
        return false;
    }
    ...
    stree->initTree(max_cells,max_len);
    ...
    while(!bin_reader.eof())
    {
        ...
        if(tmp_vol == 0)
        {
            cout << "ERROR: A voronoi cell requires a non-zero volume                     \n";
            return false;
        }
        ...
        //cout << line_counter << " " << nr_neighbors << endl;
        ...
        {
            cout << "ERROR: Failed attempt to add voronoi cell to the search octree!                     \n";
            return false;
        }
    ...
    {
        cout << "ERROR: Nr. of read in cells do not match the maximal number of expected cells!  \n";
        return false;
    }
...
bool CGridVoronoi::writeGNUPlotFiles(string path, parameter & param)
{
    ...
    plt_rat = (data_pos_aalg != uint(-1)); // 5
    plt_delta = false; //param.getPlot(plIDdelta) && (data_pos_tg != uint(-1)) && (data_pos_mx != uint(-1)) && (data_pos_td != uint(-1)); // 6
    plt_larm = false; //param.getPlot(plIDlarm) && (data_pos_tg != uint(-1)) && (data_pos_mx != uint(-1)) && (data_pos_td != uint(-1)); // 7
    plt_mach = false; //param.getPlot(plIDmach) && (data_pos_vx != uint(-1)) && (data_pos_tg != uint(-1)); // 8
    ...
    {
        cout << "ERROR: Can't write to " << grid_filename << "          \n";
        return false;
    }
    ...
    if(plt_gas_dens)
    {
        ...
        {
            cout << "ERROR: Can't write to " << dens_gas_filename << "          \n";
            return false;
        }
    }
    ...
    if(plt_dust_dens)
    {
        ...
        {
            cout << "ERROR: Can't write to " << dens_dust_filename << "          \n";
            return false;
        }
    }
    ...
    if(plt_rat)
    {
        ...
        {
            cout << "ERROR: Can't write to " << rat_filename << "          \n";
            return false;
        }
    }
    ...
    if(plt_delta)
    {
        ...
        {
            cout << "ERROR: Can't write to " << delta_filename << "          \n";
            return false;
        }
    }
    ...
    if(plt_larm)
    {
        ...
        {
            cout << "ERROR: Can't write to " << larm_filename << "          \n";
            return false;
        }
    }
    ...
    if(plt_mach)
    {
        ...
        {
            cout << "ERROR: Can't write to " << mach_filename << "          \n";
            return false;
        }
    }
    ...
    if(plt_mag)
    {
        ...
        {
            cout << "ERROR: Can't write to " << mag_filename << "          \n";
            return false;
        }
    }
    ...
    if(plt_vel)
    {
        ...
        {
            cout << "ERROR: Can't write to " << vel_filename << "          \n";
            return false;
        }
    }
    ...
    basic_grid << xm + max_len << " " << xm + max_len << " " << xm + max_len << " "
            << -max_len << " " << 0 << " " << 0 << " \n";
    basic_grid << xm + max_len << " " << xm + max_len << " " << xm + max_len << " "
            << 0 << " " << -max_len << " " << 0 << " \n";
    basic_grid << xm + max_len << " " << xm + max_len << " " << xm + max_len << " "
    ...
    basic_grid << xm + max_len << " " << xm + max_len << " " << xm << " "
            << -max_len << " " << 0 << " " << 0 << " \n";
    basic_grid << xm + max_len << " " << xm + max_len << " " << xm << " "
            << 0 << " " << -max_len << " " << 0 << " \n";
    ...
    basic_grid << xm << " " << xm + max_len << " " << xm + max_len << " "
            << 0 << " " << -max_len << " " << 0 << " \n";
    basic_grid << xm << " " << xm + max_len << " " << xm + max_len << " "
            << 0 << " " << 0 << " " << -max_len << " \n";


    basic_grid << xm + max_len << " " << xm << " " << xm + max_len << " "
            << -max_len << " " << 0 << " " << 0 << " \n";
    basic_grid << xm + max_len << " " << xm << " " << xm + max_len << " "
            << 0 << " " << 0 << " " << -max_len << " \n";

    //Delenay lines
    for(uint i = 0; i < uint(max_cells); i++)
    {
    ...
    point_fields[0] << "set style line 2 pt 7 ps 1 lc rgb 0xff0000\n";
    point_fields[0] << "set title \'3D voronoi grid geometry\' font \'Arial,12\'\n";
    point_fields[0] << "splot '-' with vectors as 3,'-' with vectors as 2,'-' with vectors as 1, '-' w p ls 2\n";
    ...
    point_fields[4]
            << "set palette defined (0 0.05 0 0, 0.4 1 0 0, 0.7 1 1 0, 1 1 1 0.5)\n";
    ...
    vec_fields[0] << "set palette defined (0 1 0 0, 0.5 0.0 0.9 0,  0.75 0.0 0.9 1, 0.9 0 0.1 0.9)\n";
    ...
    vec_fields[0]
            << "set title \'3D mag. field distribution (min:" << log10(min_mag) << " log10([T]); max:" << log10(max_mag) << " log10([T])  \' font \'Arial,12\'\n";
    ...
    vec_fields[0] << "splot  \'-\' with vectors as 3, \'-\' with vectors as 2, \'-\' with vectors as 1\n";

    //1 vel
    vec_fields[1] << vec_header.str();
    vec_fields[1] << "set palette defined (0 1 0 0, 0.5 0.0 0.9 0,  0.75 0.0 0.9 1, 0.9 0 0.1 0.9)\n";
    ...
    vec_fields[1]
            << "set title \'3D vel. field directions (min:" << log10(min_vel) << " log10(m/s); max:" << log10(max_vel) << " log10(m/s)\' font \'Arial,12\'\n";
    ...
    vec_fields[1] << "splot  \'-\' with vectors as 3, \'-\' with vectors as 2, \'-\' with vectors as 1\n";
...
bool CGridVoronoi::saveBinaryGridFile(string filename, ushort id, ushort data_size)
{
    ...
    else
    {
        cout << "Error: Cannot save voronoi grid file to:\n";
        cout << filename;
        cout << "A spherical grid requires an ID of " << GRID_ID_SPH << "!               \n";
        return false;
    }
    ...
    cout << "- Writing octree grid file      : done     \n";
...
bool CGridVoronoi::createArtificialGrid(string path)
{
    ...
    /*double tmpX[] = {-0.588, 0.79, 0.944, 0.114, 0.946, -0.293, -0.578, -0.98, 0.906, -0.387, -0.008, -0.987, 0.144, -0.619, -0.509, -0.159, 0.823, -0.611, 0.35, 0.814, 0.948, 0.219, 0.802, -0.366, -0.601, -0.693, 0.411, -0.972, 0.413, 0.547, -0.531, -0.409, 0.703, 0.748, 0.841, 0.926, -0.288, -0.379, 0.675, 0.777, 0.541, 0.959, 0.042, 0.938, 0.024, -0.206, -0.913, -0.272, 0.105, -0.431, -0.067, -0.857, 0.421, -0.435, -0.789, -0.242, -0.488, -0.115, 0.904, -0.018, 0.667, -0.575, 0.852, 0.646, 0.558, 0.428, 0.18, 0.528, 0.091, 0.545, -0.335, 0.993, 0.486, -0.814, -0.968, -0.953, 0.024, 0.052, -0.142, 0.548, 0.692, -0.779, 0.28, -0.71, 0.817, -0.93, 0.811, 0.549, -0.83, 0.579, -0.244, 0.37, 0.845, -0.968, 0.579, -0.958, -0.15, 0.181, -0.435, -0.909, 0.307, -0.126};
    double tmpY[] = {0.138, 0.311, -0.751, -0.067, -0.857, 0.194, 0.515, -0.234, -0.25, -0.212, 0.2, -0.01, -0.067, -0.953, 0.994, -0.3, 0.123, 0.144, 0.049, 0.662, -0.347, -0.333, 0.811, -0.809, -0.317, 0.525, 0.347, 0.737, 0.087, -0.1, -0.153, -0.934, 0.868, -0.499, -0.1, 0.208, -0.382, -0.354, -0.941, 0.606, -0.925, -0.334, -0.631, -0.883, 0.958, 0.884, -0.405, 0.216, -0.11, -0.333, -0.793, -0.835, 0.208, 0.145, -0.831, -0.037, 0.05, 0.779, -0.116, 0.453, -0.614, -0.47, 0.951, -0.484, -0.272, 0.486, 0.999, -0.117, -0.213, 0.77, 0.436, 0.862, 0.713, 0.359, -0.083, -0.656, 0.525, 0.052, -0.857, 0.688, -0.647, -0.654, 0.832, -0.982, -0.949, -0.902, 0.122, -0.826, 0.032, -0.42, -0.729, -0.508, 0.773, -0.937, 0.88, 0.951, 0.762, 0.311, 0.481, 0.059, 0.815, 0.184};
    double tmpZ[] = {0.022, 0.131, 0.237, -0.89, -0.65, -0.019, -0.433, -0.039, -0.537, -0.867, 0.272, -0.417, -0.168, 0.766, 0.909, 0.103, -0.382, 0.385, -0.866, -0.078, -0.878, 0.112, -0.956, -0.908, -0.915, -0.214, 0.91, 0.053, -0.898, 0.764, -0.583, 0.084, -0.105, 0.087, 0.77, 0.294, -0.36, -0.623, 0.464, -0.637, 0.211, 0.277, 0.145, 0.646, -0.454, 0.755, -0.628, 0.019, 0.078, -0.316, -0.241, 0.426, 0.795, 0.913, 0.87, 0.753, -0.653, 0.564, -0.547, 0.12, 0.328, 0.493, -0.299, 0.846, -0.986, -0.023, -0.973, 0.807, -0.31, -0.857, -0.471, 0.059, 0.01, 0.527, -0.746, 0.74, 0.738, -0.379, 0.408, 0.178, -0.387, -0.525, -0.813, 0.547, -0.537, -0.193, -0.669, 0.384, 0.527, 0.215, 0.568, 0.914, 0.746, 0.764, 0.815, 0.913, 0.035, 0.209, -0.759, -0.178, 0.094, 0.587};
    max_cells = 102; */

    /*double tmpX[] = {-0.14, 0.91, 0.87, -0.02, 0.16, 0.98, -0.62, -0.42, 0.59, -0.29, 0.89, -0.29, -0.59, -0.03, 0.06, 0.00, 0.73, 0.31, 0.93, -0.84, 0.76, 0.17, -0.06, 0.44, -0.36, 0.05, -0.56, -0.85, -0.61};
    double tmpY[] = {0.25, 0.30, 0.77, 0.80, -0.21, 0.62, 0.12, -0.62, 0.19, 0.83, 0.49, -0.97, 0.95, 0.15, -0.43, -0.79, -0.57, -0.02, -0.61, 0.88, -0.24, -0.05, -0.21, -0.20, 0.07, 0.85, -0.21, -0.53, -0.11};
    double tmpZ[] = {0.53, 0.28, 0.67, 0.70, -0.39, 0.28, -0.52, 0.62, 0.73, 0.70, -0.55, 0.00, -0.23, 0.56, -0.88, -0.61, -0.11, 0.00, -0.58, 0.81, 0.16, 0.35, 0.92, 0.36, 0.78, 0.89, 0.49, -0.15, 0.15};
    max_cells = 29; */
...
void CGridVoronoi::printParameter()
{
    if(max_cells == 0)
    {
...
bool CGridVoronoi::positionPhotonInGrid(photon_package * pp)
{
    ...
    if(cell==0)
    {
        cout << "ERROR: Photon package cannot be psitioned in Voronoi grid!                        \n";
        return false;
    }
...
bool CGridVoronoi::findStartingPoint(photon_package *pp)
{
    ...
    Vector3D dir = pp->getDirection();
    Vector3D pos = pp->getPosition(); //+0.5*dir*max_len;
