...
bool CSourceISRF::initSource(uint id, uint max, bool use_energy_density)
{
    ...
    for(uint w = 0; w < getNrOfWavelength(); w++)
    {
        ...
        double sp_energy = pl * PI * 3 * pow(radius, 2);
        if(g_zero > 0)
...
void CSourceISRF::createNextRay(photon_package * pp, ullong i_pos, ullong nr_photons)
{
    ...
    if(pp->getWavelengthID() != MAX_UINT)
    {
        ...
        energy = pl * PI * 3 * pow(radius, 2) / nr_photons;
        if(g_zero > 0)
    ...
    l.setX(e.X() * sqrt(3) * radius / 2);
    l.setY(e.Y() * sqrt(3) * radius / 2);
    l.setZ(e.Z() * sqrt(3) * radius / 2);
