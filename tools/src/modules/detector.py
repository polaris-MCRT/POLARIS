...
class DetectorChooser:
    ...
    def get_module(self):
        ...
        if self.parse_args.detector in self.detector_dict.keys():
            ...
            raise ValueError('Detector name not known! You can add a new detector in detector.py')
        detector = self.detector_dict[detector_name](self.model, self.parse_args)
        # Overwrite default values with user input
        if self.parse_args.wavelength is not None:
            if len(self.parse_args.wavelength) == 1:
                detector.parameter['wavelength_min'] = float(self.parse_args.wavelength[0])
                detector.parameter['wavelength_max'] = float(self.parse_args.wavelength[0])
                detector.parameter['nr_of_wavelength'] = 1
            elif len(self.parse_args.wavelength) == 2:
                detector.parameter['wavelength_min'] = float(self.parse_args.wavelength[0])
                detector.parameter['wavelength_max'] = float(self.parse_args.wavelength[1])
                detector.parameter['nr_of_wavelength'] = 2
            elif len(self.parse_args.wavelength) == 3:
                detector.parameter['wavelength_min'] = float(self.parse_args.wavelength[0])
                detector.parameter['wavelength_max'] = float(self.parse_args.wavelength[1])
                detector.parameter['nr_of_wavelength'] = int(self.parse_args.wavelength[2])
            else:
                raise ValueError('Too many wavelengths IDs!')
        if self.parse_args.rot_angle_1 is not None:
            detector.parameter['rot_angle_1'] = self.math.parse(self.parse_args.rot_angle_1, 'angle')
        if self.parse_args.rot_angle_2 is not None:
            detector.parameter['rot_angle_2'] = self.math.parse(self.parse_args.rot_angle_2, 'angle')

        if self.parse_args.obs_pos is not None:
            detector.parameter['obs_position_x'] = self.math.parse(self.parse_args.obs_pos[0], 'length')
            detector.parameter['obs_position_y'] = self.math.parse(self.parse_args.obs_pos[1], 'length')
            detector.parameter['obs_position_z'] = self.math.parse(self.parse_args.obs_pos[2], 'length')
        if self.parse_args.obs_vel is not None:
            detector.parameter['obs_velocity_x'] = self.math.parse(self.parse_args.obs_vel[0], 'velocity')
            detector.parameter['obs_velocity_y'] = self.math.parse(self.parse_args.obs_vel[1], 'velocity')
            detector.parameter['obs_velocity_z'] = self.math.parse(self.parse_args.obs_vel[2], 'velocity')
        ...
        if self.parse_args.max_velocity is not None:
            detector.parameter['max_velocity'] = self.math.parse(self.parse_args.max_velocity, 'velocity')
        ...
        if self.parse_args.map_shift_x is not None:
            detector.parameter['map_shift_x'] = self.math.parse(self.parse_args.map_shift_x, 'length')
        if self.parse_args.map_shift_y is not None:
            detector.parameter['map_shift_y'] = self.math.parse(self.parse_args.map_shift_y, 'length')
